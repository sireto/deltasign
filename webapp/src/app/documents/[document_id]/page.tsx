...
import { useGetContractByIdQuery, usePatchContractMutation, useSignContractMutation } from "../api/contracts";
...
const signatureFont = localFont({ src: '../../../../public/fonts/Madeva Suarte Signature Font.ttf'})
      ...
      >
        ...
        {annotations.map((ann: any) => (
              <div
                ...
                className="absolute border-silicon ring-silicon/20  rounded-[8px] border-[1.5px] ring-[2px] ring-offset-[1px]"
                >
                <div className="absolute -top-4 text-silicon px-1 text-[10px] font-medium text-midnight-gray-900 rounded">
                  {ann.signer == userEmail ? "You" : ann.signer}
                </div>
                <div className={`${signatureFont.className} text-black w-full h-full flex justify-center items-center itallic text-4xl`} ref={signatureRef}>
                  {
                    userHasSigned && ann.signer == userEmail && <span>{previewSignature}</span>
                  }
                </div>
              </div>
        ))}
...
export default function Page() {
  ...
  const webToPdf = (webX: number, webY: number, webWidth: number, webHeight: number) => {
  ...
  const pdfToWeb = (pdfX1: number, pdfY1: number, pdfX2: number, pdfY2: number) => {
...
const { data: contract } = useGetContractByIdQuery({ uuid: contractId });
  ...
  const [userHasSigned , setUserHasSigned] = useState(false)
    ...
    const patchContractData: PatchContractRequest = {
      ...
      annotations: annotations.map((annotation) => {
        const pdfCoords = webToPdf(annotation.x, annotation.y, annotation.width, annotation.height);
        
        console.log("📍 Web → PDF Conversion:");
        console.log("Web:", { x: annotation.x, y: annotation.y, width: annotation.width, height: annotation.height });
    ...
    if (contract) {
      setAnnotations(
        contract.annotations.map((annotation: any, index: number) => {
          const webCoords = pdfToWeb(annotation.x1, annotation.y1, annotation.x2, annotation.y2);
          
          console.log("📍 PDF → Web Conversion:");
          console.log("PDF:", { x1: annotation.x1, y1: annotation.y1, x2: annotation.x2, y2: annotation.y2 });
 ...
 const handleExport = async () => {
    ...
    try {
        const dataUrl = await toPng(signatureRef.current, { cacheBust: true, skipFonts: false , width: 154 , height: 44 , canvasHeight : 44 , canvasWidth : 154,  style : {
          width : '154px',
          height : '44px'
        } });
        ...
        const result = await signContract({ uuid: contractId, formData }).unwrap();
  ...
  const SignDocumentButton = () => {
    return (
      <Button onClick={()=>{setShowSignContractDialog(true)}}>
        <Pen/>
         Sign Document
      </Button>
    )
  ...
  return (
    <div>
      ...
      <Dialog open={showSignContractDialog} onOpenChange={setShowSignContractDialog}>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 p-4 bg-[#F9F9FC]">
            ...
            <Button
              onClick={async () => { await handleAcceptAndSignDocument()}}
      ...
      <NavBar className="sticky top-0" 
              ...
              RenderButton={contract.status == "draft" ? ShareDocumentButton : (contract.status == "fully signed" ? DownloadDocumentButton : SignDocumentButton)}
      />
          ...
          <div className="no-scrollbar flex-1 overflow-hidden rounded-[12px] border border-gray-200">
            ...
            >
              <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.11/build/pdf.worker.min.js">
                <Viewer
                  fileUrl={contract.status === "fully signed" ? contract.signed_doc_url :contract.document.s3_url}
                  plugins={[
